layui.define(['table', 'jquery', 'form'], function (exports) {
    ...
    */
    tableSelect.prototype.render = function (opt) {
        ...
        //默认设置
        opt.searchUrl = opt.searchUrl;
        opt.fixedValue = opt.fixedValue || null;
        ...
        elem.off('click').on('click', function(e) {
            ...
            form.on('submit(tableSelect_btn_search)', function(data){
                ...
                var inKey;
                if(opt.fixedValue != null){
                    for(let key in opt.fixedValue){
                        inKey = key
                    }
                    params[inKey] = opt.fixedValue[inKey];
                }
                CoreUtil.sendAjax(opt.searchUrl,JSON.stringify(params),function (res) {
                    opt.table.data = res.data;
                    tableSelect_table.reload(opt.table);
                },"POST",false, function (res) {
                    //无权限访问操作日志列表提示
                    layer.msg("抱歉！您暂无权限");
                    let noAuthorityData=[];
                    opt.table.data = noAuthorityData;
                    tableSelect_table.reload(opt.table);
                });
                console.log(data);
                return false;
